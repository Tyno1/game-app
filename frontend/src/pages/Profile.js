  ...
  return (
    <div className="w-full min-h-[100vh] bg-black pt-14 md:pt-20 flex gap-8 flex-col justify-center items-center">
      ...
      <div className="container w-full flex flex-col gap-10 w-[50%] px-10 py-20 items-start rounded-3xl justify-center bg-stone-900">
        <div className="name flex flex-wrap gap-10 items-center w-full">
          <p className="text-amber-500 text-2xl">Name:</p>
          <p className="text-white text-4xl">{user.user.fullName}</p>
        </div>
        <div className="name flex flex-wrap gap-10 items-center w-full">
          <p className="text-amber-500 text-2xl">Username:</p>
          <p className="text-white text-4xl">{user.user.username}</p>
        </div>
        <div className="name flex flex-wrap gap-10 items-center">
          <p className="text-amber-500 text-2xl">Email:</p>
          <p className="text-white text-4xl">{user.user.email}</p>
        </div>
      </div>
    </div>
  );
